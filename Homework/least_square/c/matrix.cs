...
public bool approx(vector b,double acc=1e-6, double eps=1e-6){
	if(this.size1!=b.size)return false;
	if(this.size2!=1)return false;
	for(int i=0;i<size1;i++){
        if(!approx(this[i,1],b[i],acc,eps))
            return false;
    }
	return true;
}
